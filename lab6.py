...
def register():
    ...
    # users.query: Это объект запроса к таблице базы данных, соответствующей модели users.
    #.filter_by(username=username_form): Этот метод фильтрует результаты запроса, оставляя только те записи, у которых значение колонки username 
    #.first(): Этот метод возвращает первую запись, соответствующую условиям фильтрации. 
...
def view_article(article_id):
    ...
    # Запрос к базе данных для получения статьи с указанным article_id. Если статья не найдена, вместо возможного исключения None возвращается HTTP-ответ с кодом ошибки 404
...
# Определение нового маршрута в Flask-приложении.
